			...
			<div style="margin-top: 20px;" class="col-12">
				<div class="module">
					<table class="table" id="finan_export">
						...
						$consulta = "SELECT tickets_saled.id, events.title AS evento, tickets_saled.ticket, users.nome AS cliente, tickets_saled.qntd, tickets_saled.valor * tickets_saled.qntd AS total, tickets_saled.status, tickets_saled.invoice
						             FROM tickets_saled
						             INNER JOIN events ON tickets_saled.event_id = events.id
						             INNER JOIN users ON tickets_saled.user_id = users.id LIMIT 50";
						...
						<tr>
						    ...
						    <td class="align-middle left"><?php echo $dado['cliente']; ?></td>
						    <td class="align-middle center"><?php echo $dado['qntd']; ?></td>
						    <td class="align-middle center"><?php echo 'R$ ' . number_format($dado['total'], 2,',','.'); ?></td>
						    <td class="align-middle center"><?php $stats = statusConvert($dado['status']);echo '<label class="stats" style="border-color: '.$stats["color"].'; color: '.$stats["color"].';">'.$stats["message"].'</label>';?></td>
